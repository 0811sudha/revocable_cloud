...
$rs = mysqli_query($link, "select name,userid,skey from newpatient where accept!='pending' and userid not in(select userid from docrequest where doctorid='$_SESSION[doctorid]') order by id") or die(mysqli_error($link));
echo "<div class='scrolldiv'><table class='report_tab' style='float:none;margin:auto;min-width:700px;'><thead><tr><th colspan='3'>KEY REQUEST TO PATIENT<tr><th>Name<th>Email<th>Task</thead><tbody>";
                    while($r = mysqli_fetch_row($rs)) {
                        echo "<tr>";
		echo "<td>".AesCtr::decrypt($r[0],AesCtr::decrypt($r[2],'abc',256),256)."<td>$r[1]";
...
$rs = mysqli_query($link, "select n.name,d.userid,accstatus,skey from docrequest d,newpatient n where d.doctorid='$_SESSION[doctorid]' and d.userid=n.userid") or die(mysqli_error($link));
echo "<div class='scrolldiv'><table class='report_tab' style='float:none;margin:auto;min-width:700px;'><thead><tr><th colspan='4'>KEY REQUEST STATUS<tr><th>Name<th>Email<th>Status<th>Task</thead><tbody>";
                    while($r = mysqli_fetch_row($rs)) {
                        echo "<tr>";
                        echo "<td>".AesCtr::decrypt($r[0],AesCtr::decrypt($r[3],'abc',256),256)."<td>$r[1]<td>$r[2]";
